...
public class DList extends List {
    ...
    public DList connect(DList l){
        ListNode n1 = this.back();
        ListNode n2 = l.front();
        ListNode n3 = l.back();
        List result;
        if(!n1.isValidNode() && !n2.isValidNode()) {
            result = this;
        } else if(n1.isValidNode() && !n2.isValidNode()) {
            result = this;
        } else if(!n1.isValidNode() && n2.isValidNode()) {
            result = l;
        } else if(n1.isValidNode() && n2.isValidNode()) {
            ((DListNode) n1).next = (DListNode) n2;
            ((DListNode) n2).prev = (DListNode) n1;
            ((DListNode) n3).next = head;
            head.prev = (DListNode) n3;
            l.head.next = null;
            l.head.prev = null;
            size += l.size;
            l.size = 0;
